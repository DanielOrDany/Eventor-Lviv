...
public class Bot extends TelegramLongPollingBot {
    ...
    public void setButton(SendMessage sendMessage)
    {
       ...
       /* public void sendMsgStart(long chat_id) {
            SendMessage message_text = new SendMessage()
                    .setChatId(chat_id)
                    .setText("Eventor готовий допомогти тобі")
                    .enableMarkdown(true);
            InlineKeyboardMarkup markupInline = new InlineKeyboardMarkup();
            List<List<InlineKeyboardButton>> rowsInline = new ArrayList<>();
            List<InlineKeyboardButton> rowInline1 = new ArrayList<>();
            rowInline1.add(new InlineKeyboardButton().setText("Знайти подію"));
            rowsInline.add(rowInline1);
            markupInline.setKeyboard(rowsInline);
            message_text.setReplyMarkup(markupInline);
            try {
                execute(message_text);
                setButton(message_text);
            } catch (TelegramApiException e) {
                e.printStackTrace();
            }
        }*/
